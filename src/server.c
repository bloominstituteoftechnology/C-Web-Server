...
{
  // !!!! IMPLEMENT ME
  int rnd = getRand(1, 20);
  // char c = (char)rnd;
  char stringRepresentationOfNumber[3];
  sprintf(stringRepresentationOfNumber, "%d", rnd); // Now str contains the integer as characters
  // printf("getRand() %s\n", str);
  send_response(fd, "HTTP/1.1 200 OK", "text/plain", stringRepresentationOfNumber);
}
...
void get_date(int fd)
{
  ...
  printf("getRand() %s\n", s);
  send_response(fd, "HTTP/1.1 200 OK", "text/plain", s);
}
...
void handle_http_request(int fd)
{
  ...
  // why will request_path == "/" always be false???
